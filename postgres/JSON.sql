-- create table that will hold the json data

DROP TABLE IF EXISTS jtrack CASCADE;
CREATE TABLE IF NOT EXISTS jtrack (id SERIAL, body JSONB);

-- JSON import with copy, is often easier with Python, but for simple JSON without embedded newlines in the JSON
-- values, this is good enough.
...
\copy jtrack (body) FROM './library.txt' WITH (FORMAT text, HEADER false);

SELECT * FROM jtrack LIMIT 5;
SELECT pg_typeof(body) FROM jtrack LIMIT 1;

-- query from json where key is 'name' and parse to str
...
-- casting string to integer to use number functions
...
SELECT * FROM jtrack ORDER BY id DESC LIMIT 5;
...
-- btree index on string value of json name entry
CREATE INDEX jtrack_btree ON jtrack USING BTREE ((body->>'name'));
-- inverted index on the json itself, it remembers the keys
CREATE INDEX jtrack_gin ON jtrack USING gin (body);
--inverted index on the json itself, but remembers key-value pairs
...
-- create an inverse index into the json, storing key value data
...
SELECT DISTINCT substring(body->>'url', 'https://swapi.py4e.com/api/([a-z]+)/') FROM swapi;

SELECT DISTINCT ('{"type": "' || substring(body->>'url', 'https://swapi.py4e.com/api/([a-z]+)/') || '" }')
FROM swapi;
SELECT DISTINCT ('{"type": "' || substring(body->>'url', 'https://swapi.py4e.com/api/([a-z]+)/') || '" }')::jsonb
FROM swapi;
